...
const isValidDateDisplay = (value: string) => {
...
/**
 * Existing records may not have netWeightPerPackage
 * because this field was added later.
 *
 * For old data:
 * - If packageQty > 0, derive per-package weight
 *   from the existing total netWeight.
 * - This preserves the old total net weight.
 */
const normalizeInitialItems = (
  ...
  return purchase.items.map((item) => {
    ...
    const storedNetWeight = Number(
      item.netWeight || 0
    );

    const existingPerPackage = Number(
      item.netWeightPerPackage || 0
    );

    const netWeightPerPackage =
      existingPerPackage > 0
        ? existingPerPackage
        : packageQty > 0 &&
          storedNetWeight > 0
        ? storedNetWeight / packageQty
        : 0;

    const calculatedNetWeight =
      packageQty > 0 &&
      netWeightPerPackage > 0
        ? packageQty * netWeightPerPackage
        : storedNetWeight;
...
export default function ExportPurchaseForm({
  ...
  const [purchaseDateDisplay, setPurchaseDateDisplay] =
    useState(
      displayDate(
        initialData?.purchaseDate ||
          todayISO()
  ...
  const [
    supplierInvoiceDateDisplay,
    setSupplierInvoiceDateDisplay,
  ] = useState(
    displayDate(
  ...
  const [
    expectedReceiptDateDisplay,
    setExpectedReceiptDateDisplay,
  ] = useState(
    displayDate(
  ...
  const [status, setStatus] =
    useState<ExportPurchaseStatus>(
      initialData?.status || "Draft"
    );

  const [items, setItems] =
    useState<ExportPurchaseItem[]>(
  ...
  const [customsDuty, setCustomsDuty] =
    useState(
      initialData?.customsDuty ?? 0
    );

  const [portCharges, setPortCharges] =
    useState(
    ...
    setItems((current) =>
      current.map(
        (item, itemIndex) => {
          ...
          /*
           * Flexible packing logic:
           *
           * Package Qty × Net Weight / Package
           * = Total Net Weight
           *
          ...
          /*
           * If an old record is edited and only
           * Net Weight is available, keep it.
           *
           * New records use the automatic
           * package calculation above.
           */
  ...
  const resetForm = () => {
    ...
    setPurchaseDate(nextToday);
    setPurchaseDateDisplay(
      displayDate(nextToday)
    );
  ...
  const handlePurchaseDateTextChange = (
    value: string
  ) => {
    const cleaned = value
      .replace(/\\D/g, "")
    ...
    setPurchaseDateDisplay(formatted);

    if (formatted.length === 10) {
      if (!isValidDateDisplay(formatted)) {
        setPurchaseDate("");
        return;
      }

      setPurchaseDate(
        formatDateForStorage(formatted)
      );
      return;
    }
  ...
  const handleSupplierInvoiceDateTextChange = (
    value: string
  ) => {
    const cleaned = value
      .replace(/\\D/g, "")
    ...
    setSupplierInvoiceDateDisplay(formatted);
  ...
  const handleExpectedReceiptDateTextChange = (
    value: string
  ) => {
    const cleaned = value
      .replace(/\\D/g, "")
    ...
    setExpectedReceiptDateDisplay(formatted);
  ...
  const handleSubmit = (
    ...
    if (
      ...
      alert(
        "Purchase Date is required in DD/MM/YYYY format."
      );
      return;
    }
    ...
    if (
      ...
      alert(
        "Supplier Invoice Date must be in DD/MM/YYYY format."
      );
      return;
    }
    ...
    if (
      ...
      alert(
        "Expected Receipt Date must be in DD/MM/YYYY format."
      );
      return;
    }
     ...
     * Packing validation.
     *
     * If Package Qty is entered, Weight / Package
     * must also be entered.
     *
     * If Weight / Package is entered, Package Qty
     * must also be entered.
     */
    ...
    if (invalidPacking) {
      alert(
        "Please enter both Package Qty and Net Weight / Package."
      );
      return;
    }
          ...
          <div>
            ...
            >
              ...
              <input
                ...
                aria-label="Purchase Date calendar"
                style={{
                  position: "absolute",
                  right: "4px",
                  width: "22px",
          ...
          <div>
            ...
            >
              <input
                ...
                onChange={(e) =>
                  handleSupplierInvoiceDateTextChange(
                    e.target.value
                  )
                }
                placeholder="DD/MM/YYYY"
              ...
              <input
                ...
                aria-label="Supplier Invoice Date calendar"
                style={{
                  position: "absolute",
                  right: "4px",
                  width: "22px",
          ...
          <div>
            ...
            >
              <input
                ...
                onChange={(e) =>
                  handleExpectedReceiptDateTextChange(
                    e.target.value
                  )
                }
                placeholder="DD/MM/YYYY"
              ...
              <input
                ...
                aria-label="Expected Receipt Date calendar"
                style={{
                  position: "absolute",
                  right: "4px",
                  width: "22px",
  ...
  status === "Cancelled"
        ...
        >
          Packing is flexible as per
          requirement. Total Net Weight =
          Package Qty × Net Weight / Package.
          Example: 3 Bags × 25 KG = 75 KG.
        </div>
      </div>
